...
| `packages/map-adapters/` | SVG map surface, overlay conversion, MapLibre preview boundary, and transient generated field-pivot advisory overlays. | Keep rendering read-only unless an explicit editor mutation path is implemented through core reducer actions. Generated advisory overlay features must not write project geometry, storage, archives, or exports. |
