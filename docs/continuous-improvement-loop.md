...
| 004 | Add reducer coverage proving draft cancel clears only draft state and preserves the map viewport. | Existing `reduceDrawingMapState` draft and viewport semantics. | `packages/geometry/src/mapInteraction.test.ts`; loop ledger. | `npm run validate`; `npm test -w @cplayout/geometry`; `git diff --check`; `npm audit`. | `mapInteraction.test.ts` c8916159; ledger 23495944. | Pending until committed. | Pass; clearing draft increments draft revision while preserving viewport. | Improve Review Layout no-mutation browser proof. |
