...
| Center-pivot awareness and advisory mapping | Core schemas, KML/XML import/export, map tools, Design Builder UI, and advisory geometry now preserve planning boundaries, machine zones, linear move paths, measurement lines, well locations, underground wire paths, and existing pivot evidence as projected-XY map/survey data. Ideal-center candidates expose minimum-clearance gates, operator-supplied cost-input status, obstacle-crossing profile notices, transient machine-zone reviews, projected-XY multi-zone collision/review zone evidence with severity flags, advisory current/full-circle machine strategy comparison, path-based advisory linear/lateral swept-strip scoring, operator-labeled second-pivot opportunity envelopes, and local cost-assumption ranking in the Calculate dialog. | No certified engineering design, runtime multi-pivot collision prevention, proprietary bender kinematics, certified linear/lateral design, automatic/vendor quote, client-ready report, Google Earth render proof, or native runtime proof is claimed from this pass. |
...
- Advisory design expansion: `analyzeIdealPivotCenter` and placement candidates now report minimum boundary/obstacle clearance gates, missing or explicit cost-input status, advisory obstacle-crossing profile notices, and transient planning-boundary/machine-zone candidate reviews without mutating canonical projected `XY`.
...
- Interface expansion: Design Builder keeps Awareness / Design Review surfaces for machine zones, planning boundaries, linear move paths, measurement lines, imported pivot evidence, wells, underground wire, outside-wet acres, hard conflicts, advisory scenario counts, modeled union acres, and envelope-risk counts; the Calculate dialog now adds local Cost Review for operator-supplied fixed/per-meter/per-tower cost assumptions and cost-ranked advisory candidates/strategies; drawing/GNSS tools expose planning boundary, machine zone, linear move path, measurement line, well, and underground wire feature capture.
- Regression coverage: synthetic KML tests mirror the local Will Rhea placemark naming pattern without committing raw client coordinates; core document/XML/KML tests cover the new map feature kinds; geometry tests cover cost-input status, machine-zone reviews, strict clearance, obstacle-crossing advisory notices, multi-zone scenario compilation, projected-XY collision/review zones, buffer-only separation warnings, machine-strategy comparison, advisory linear/lateral swept strips, explicit bender/second-pivot evidence gating, unsupported bender placeholders, and no mutation of input projects; browser proof covers local cost assumptions without dirtying project geometry.
