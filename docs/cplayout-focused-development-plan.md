...
| Center-pivot awareness and advisory mapping | Core schemas, KML/XML import/export, map tools, Design Builder UI, and advisory geometry now preserve planning boundaries, machine zones, linear move paths, measurement lines, pump/well locations, underground pipe/wire paths, power poles/lines, and existing pivot evidence as projected-XY map/survey data. Ideal-center candidates expose minimum-clearance gates, operator-supplied cost-input status, obstacle-crossing profile notices, transient machine-zone reviews, generated within-field multi-pivot advisory screening, projected-XY multi-zone collision/review zone evidence with severity flags, full-scope compiled-boundary geometry and coverage/dry-acre metrics based on explicit planning boundaries or the active field-boundary fallback, advisory current/full-circle machine strategy comparison, path-based advisory linear/lateral swept-strip scoring, operator-labeled second-pivot opportunity envelopes, local cost-assumption ranking, current/missing/stale generated review-zone audit, local advisory design report preview/export, and obstacle interaction summaries for hard-blocking, no-spray, span-clearance, tower-track, utility-path, and outside-reach evidence in the Calculate dialog. | No certified engineering design, runtime multi-pivot collision prevention, proprietary bender kinematics, certified obstacle crossing/passability, certified linear/lateral design, automatic/vendor quote, certified/final client deliverable beyond local advisory report export, Google Earth render proof, or native runtime proof is claimed from this pass. |
...
- Advisory design expansion: `analyzeIdealPivotCenter`, placement candidates, and `analyzeAdvisoryObstacleInteractions` now report minimum boundary/obstacle clearance gates, missing or explicit cost-input status, advisory obstacle-crossing profile notices, obstacle/utility interaction categories, and transient planning-boundary/machine-zone candidate reviews without mutating canonical projected `XY`.
