...
| Center-pivot awareness and advisory mapping | Core schemas, KML/XML import/export, map tools, Design Builder UI, and advisory geometry now preserve planning boundaries, machine zones, linear move paths, measurement lines, well locations, underground wire paths, and existing pivot evidence as projected-XY map/survey data. Ideal-center candidates expose minimum-clearance gates, explicit/missing cost-input status, obstacle-crossing profile notices, transient machine-zone reviews, projected-XY multi-zone collision/review zone evidence with severity flags, advisory current/full-circle machine strategy comparison, and path-based advisory linear/lateral swept-strip scoring. | No certified engineering design, runtime multi-pivot collision prevention, bender kinematics, certified linear/lateral design, automatic quote, client-ready report, Google Earth render proof, or native runtime proof is claimed from this pass. |
