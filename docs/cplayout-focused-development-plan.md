...
| Center-pivot awareness and advisory mapping | Core schemas, KML/XML import/export, map tools, Design Builder UI, and advisory geometry now preserve planning boundaries, machine zones, measurement lines, well locations, underground wire paths, and existing pivot evidence as projected-XY map/survey data. Ideal-center candidates expose minimum-clearance gates, explicit/missing cost-input status, obstacle-crossing profile notices, and transient machine-zone reviews. | No certified engineering design, multi-pivot collision prevention, bender kinematics, linear/lateral design, automatic quote, client-ready report, Google Earth render proof, or native runtime proof is claimed from this pass. |
...
- Interface expansion: Design Builder now includes an Awareness / Design Review step for machine zones, planning boundaries, measurement lines, imported pivot evidence, wells, underground wire, outside-wet acres, and hard conflicts; drawing/GNSS tools expose planning boundary, machine zone, measurement line, well, and underground wire feature capture.
- Regression coverage: synthetic KML tests mirror the local Will Rhea placemark naming pattern without committing raw client coordinates; core document/XML tests cover the new map feature kinds; geometry tests cover cost-input status, machine-zone reviews, strict clearance, and obstacle-crossing advisory notices.
