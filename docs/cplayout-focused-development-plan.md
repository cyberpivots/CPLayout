...
- Interface expansion: Design Builder keeps Awareness / Design Review surfaces for machine zones, planning boundaries, linear move paths, measurement lines, imported pivot evidence, pump/well locations, underground pipe/wire, powerline/pole evidence, outside-wet acres, hard conflicts, advisory obstacle interaction summaries, advisory scenario counts, modeled union acres, full-scope coverage/dry-acre metrics, and envelope-risk counts; the Calculate dialog now adds local Cost Review for operator-supplied fixed/per-meter/per-tower cost assumptions, cost-ranked advisory candidates/strategies, full-scope boundary review, and obstacle interaction review; drawing/GNSS tools expose planning boundary, machine zone, linear move path, measurement line, pump, well, pipeline, underground wire, power line, and power pole feature capture.
- Regression coverage: synthetic KML tests mirror the local Will Rhea placemark naming pattern without committing raw client coordinates; core document/XML/KML tests cover the new map feature kinds; geometry tests cover cost-input status, machine-zone reviews, strict clearance, obstacle-crossing advisory notices, obstacle interaction categories, full-scope compiled-boundary metrics, multi-zone scenario compilation, projected-XY collision/review zones, buffer-only separation warnings, machine-strategy comparison, advisory linear/lateral swept strips, explicit bender/second-pivot evidence gating, unsupported bender placeholders, and no mutation of input projects; browser proof covers local cost assumptions, full-scope boundary review, and obstacle interaction display without dirtying project geometry.
