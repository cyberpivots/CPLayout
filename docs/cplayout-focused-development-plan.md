...
- Regression coverage: synthetic KML tests mirror the local Will Rhea placemark naming pattern without committing raw client coordinates; core document/XML/KML tests cover the new map feature kinds; core reducer tests cover batch upsert of generated machine-zone Circle review features without moving the active pivot; geometry tests cover cost-input status, machine-zone reviews, generated within-field multi-pivot screening, strict clearance, obstacle-crossing advisory notices, obstacle interaction categories, full-scope compiled-boundary metrics, multi-zone scenario compilation, projected-XY collision/review zones, buffer-only separation warnings, machine-strategy comparison, advisory linear/lateral swept strips, explicit bender/second-pivot evidence gating, unsupported bender placeholders, and no mutation of input projects; map-adapter tests cover generated advisory GeoJSON layer types and MapLibre style layers; browser proof covers local cost assumptions, generated field-pivot plan review/overlay state, explicit save-to-review-zones flow, full-scope boundary review, and obstacle interaction display without automatic canonical geometry mutation.
