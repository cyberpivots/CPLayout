...
| Center-pivot awareness and advisory mapping | Core schemas, KML/XML import/export, map tools, Design Builder UI, and advisory geometry now preserve planning boundaries, machine zones, measurement lines, well locations, underground wire paths, and existing pivot evidence as projected-XY map/survey data. Ideal-center candidates expose minimum-clearance gates, explicit/missing cost-input status, obstacle-crossing profile notices, transient machine-zone reviews, conservative multi-zone machine-envelope conflict warnings, and advisory current/full-circle machine strategy comparison. | No certified engineering design, true multi-pivot collision prevention, bender kinematics, linear/lateral design, automatic quote, client-ready report, Google Earth render proof, or native runtime proof is claimed from this pass. |
...
- Machine strategy comparison: `compareAdvisoryMachineStrategies` ranks current-machine, same-radius full-circle, and generated full-circle radius strategies with optional local cost inputs, while explicitly marking linear/lateral and bender/second-pivot concepts as unsupported until source-backed mechanics are implemented.
- Interface expansion: Design Builder now includes an Awareness / Design Review step for machine zones, planning boundaries, measurement lines, imported pivot evidence, wells, underground wire, outside-wet acres, hard conflicts, advisory scenario counts, modeled union acres, and envelope-risk counts; drawing/GNSS tools expose planning boundary, machine zone, measurement line, well, and underground wire feature capture.
- Regression coverage: synthetic KML tests mirror the local Will Rhea placemark naming pattern without committing raw client coordinates; core document/XML tests cover the new map feature kinds; geometry tests cover cost-input status, machine-zone reviews, strict clearance, obstacle-crossing advisory notices, multi-zone scenario compilation, envelope-overlap warnings, machine-strategy comparison, unsupported linear/bender placeholders, and no mutation of input projects.
