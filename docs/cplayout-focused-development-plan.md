...
- Machine strategy comparison: `compareAdvisoryMachineStrategies` ranks current-machine, same-radius full-circle, generated full-circle radius, and operator-supplied linear/lateral path strategies with optional local cost inputs, while explicitly keeping bender/second-pivot concepts unsupported until source-backed mechanics are implemented.
