...
const appliedRecommendationState = reduceProjectEditorState(createProjectEditorState(sampleProject), {
  ...
  recommendation: {
    ...
    summary: "Apply projected XY geometry after operator review.",
    proposedGeometry: {
      projectCrs: sampleProject.projectCrs,
      pivotCenter: { x: sampleProject.pivotCenter.x + 3, y: sampleProject.pivotCenter.y + 4 },
...
assert.deepEqual(appliedRecommendationState.project.pivotCenter, { x: sampleProject.pivotCenter.x + 3, y: sampleProject.pivotCenter.y + 4 });
assert.equal(appliedRecommendationState.project.fieldBoundary.length, sampleProject.fieldBoundary.length);
...
const metadataOnlyRecommendationState = reduceProjectEditorState(createProjectEditorState(sampleProject), {
  ...
  recommendation: {
    ...
    projectCrs: sampleProject.projectCrs,
    summary: "No geometry.",
    proposedGeometry: { projectCrs: sampleProject.projectCrs },
    confidence: 0.4,
    evidenceIds: [],
    reviewStatus: "unreviewed",
    warnings: [],
...
assert.match(metadataOnlyRecommendationState.lastError ?? "", /does not include projected geometry/);
assert.deepEqual(metadataOnlyRecommendationState.project.pivotCenter, sampleProject.pivotCenter);

const selfIntersectingRecommendationState = reduceProjectEditorState(createProjectEditorState(sampleProject), {
  type: "apply_model_recommendation",
  recommendation: {
    id: "rec-self-intersecting",
    projectId: sampleProject.id,
    modelName: "test-review-gated-recommender",
    modelVersion: "0.1.0",
    createdAt: "2026-05-29T00:00:00.000Z",
...
assert.match(selfIntersectingRecommendationState.lastError ?? "", /self-intersect|degenerate/);
assert.deepEqual(selfIntersectingRecommendationState.project.fieldBoundary, sampleProject.fieldBoundary);

const outsidePivotRecommendationState = reduceProjectEditorState(createProjectEditorState(sampleProject), {
  type: "apply_model_recommendation",
  recommendation: {
