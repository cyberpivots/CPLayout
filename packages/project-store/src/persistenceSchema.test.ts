...
const oldClientsTable = ["cust", "omers"].join("");
const oldProjectClientIdColumn = ["cust", "omer_id"].join("");
const oldClientsSortIndex = ["idx_", "cust", "omers", "_sort"].join("");
const oldProjectClientIndex = ["idx_project_records_", "cust", "omer"].join("");
...
assert.match(sql, new RegExp(`CREATE TABLE IF NOT EXISTS ${oldClientsTable}`));
assert.match(sql, new RegExp(`ALTER TABLE ${oldClientsTable} ADD COLUMN contact_name TEXT NOT NULL DEFAULT ''`));
assert.match(sql, new RegExp(`ALTER TABLE ${oldClientsTable} ADD COLUMN email TEXT NOT NULL DEFAULT ''`));
assert.match(sql, new RegExp(`ALTER TABLE ${oldClientsTable} ADD COLUMN phone TEXT NOT NULL DEFAULT ''`));
assert.match(sql, new RegExp(`ALTER TABLE ${oldClientsTable} ADD COLUMN location TEXT NOT NULL DEFAULT ''`));
assert.match(sql, new RegExp(`ALTER TABLE ${oldClientsTable} ADD COLUMN notes TEXT NOT NULL DEFAULT ''`));
assert.match(sql, new RegExp(`ALTER TABLE ${oldClientsTable} ADD COLUMN company_name TEXT NOT NULL DEFAULT ''`));
assert.match(sql, new RegExp(`ALTER TABLE ${oldClientsTable} ADD COLUMN primary_contact_first_name TEXT NOT NULL DEFAULT ''`));
assert.match(sql, new RegExp(`ALTER TABLE ${oldClientsTable} ADD COLUMN primary_contact_middle_initial TEXT NOT NULL DEFAULT ''`));
assert.match(sql, new RegExp(`ALTER TABLE ${oldClientsTable} ADD COLUMN primary_contact_last_name TEXT NOT NULL DEFAULT ''`));
assert.match(sql, new RegExp(`ALTER TABLE ${oldClientsTable} ADD COLUMN primary_contact_suffix TEXT NOT NULL DEFAULT ''`));
...
assert.match(sql, new RegExp(`ALTER TABLE ${oldClientsTable} RENAME TO clients`));
assert.match(sql, new RegExp(`ALTER TABLE project_records RENAME COLUMN ${oldProjectClientIdColumn} TO client_id`));
...
assert.equal(indexes.includes(oldClientsSortIndex), false);
assert.equal(indexes.includes(oldProjectClientIndex), false);
